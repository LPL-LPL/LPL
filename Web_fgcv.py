...
parser = argparse.ArgumentParser(description='PyTorch implementation of paper LPL')
...
parser.add_argument('--loss_weight3', default=0.0, type=float,
                    help='warmup jsd loss weight')
parser.add_argument('--loss_weight4', default=1.0, type=float,
...
def main():
    ...
    model_path = 'Webthree{time}ds_{ds}_lr_{lr}_ep_{ep}_ps_{ps}_lw_{lw}_warmupepochs_{warmupepochs}_rechange_per_epoch_{rechange_per_epoch}'.format(
...
def train(train_loader, model, loss_fn, loss_cont_fn, optimizer, epoch, args, tb_logger,topk_index,train_givenY,start_upd_prot=False):  
    ...
    for it, sample in enumerate(pbar):
        ...
        if epoch<args.warmupepochs:
            ...
            jocor_loss = loss_jocor(logits.to(args.gpu), logits2.to(args.gpu) , y)
        ...
        if(epoch==args.warmupepochs-1): 
            ...
            co_lambda=0.5
            loss_pick_1 = F.cross_entropy(logits.to(args.gpu), y, reduce = False) * (1-co_lambda)
            loss_pick_2 = F.cross_entropy(logits.to(args.gpu), y, reduce = False) * (1-co_lambda)
            loss_pick = (loss_pick_1 + loss_pick_2 + co_lambda * kl_loss_compute(logits.to(args.gpu), logits.to(args.gpu),reduce=False) + co_lambda * kl_loss_compute(logits.to(args.gpu), logits.to(args.gpu), reduce=False))

            small_loss= F.cross_entropy(logits.to(args.gpu), y, reduce = False)+ loss_pick *args.loss_weight3
        ...
        if epoch>=args.warmupepochs:
            ...
            loss_conf_penalty = conf_penalty(cls_out)



        if epoch<args.warmupepochs:
            loss = pnp_loss + jocor_loss*args.loss_weight3  
        else:
            loss = loss_cls + loss_cont*args.loss_weight1 + jocor_loss*args.loss_weight2 + loss_conf_penalty*args.loss_weight4 
...
def build_dataset_loader(params): 
    ...
    if params.dataset.startswith('web-'): 
        if params.dataset == 'web-bird':
            dataset = build_webfg_dataset(os.path.join("/home/jxr/Dataset/fgcv/web-data/", "web-bird"), CLDataTransform(transform['train'], transform['train_strong_aug']), transform['test']) 
        elif params.dataset == 'web-car':
            dataset = build_webfg_dataset(os.path.join("/home/jxr/Dataset/fgcv/web-data/", "web-car"), CLDataTransform(transform['train'], transform['train_strong_aug']), transform['test']) 
        elif params.dataset == 'web-aircraft':
            dataset = build_webfg_dataset(os.path.join("/home/jxr/Dataset/fgcv/web-data/", "web-aircraft"), CLDataTransform(transform['train'], transform['train_strong_aug']), transform['test'])
